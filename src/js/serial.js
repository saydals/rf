...
export const serial = {
    ...
    connectBLE: function (deviceId, options, callback) {
        ...
        const cachedBleDevice = (self.cachedBLEDevices || []).find((device) =>
            (device.address || '').toLowerCase() === String(deviceId).toLowerCase()
        );
        ...
        bleConnect(deviceId,
            function (peripheral) {
                ...
                bleWrite(deviceId, self.bleServiceUUID, self.bleTxCharUUID, exitCmd.buffer,
                    function () {
                        console.log('BLE: exit sent');
                    },
                    function (error) {
                        console.warn('BLE: exit command failed', error);
                    }
                );

                // Do not make the UI wait for the optional CLI escape write.
                // GATT/notifications are already ready at this point; MSP can
                // start immediately while the small housekeeping packet is sent.
    ...
    getDevices: function (callback) {
        ...
        // Cordova 환경: BLE + SPP + USB 시리얼 장치를 모두 병합하여 반환
        // (chrome.serial은 cordova_chromeapi.js에서 usbevent 플러그인으로 shim됨)
        if (GUI.isCordova()) {
            const allDevices = [];

            // BLE 장치 포함 (직전 스캔 결과가 cachedBLEDevices에 있음)
            if (getNordicBle() && self.cachedBLEDevices && self.cachedBLEDevices.length > 0) {
                self.cachedBLEDevices.forEach(function (device) {
                    allDevices.push({
            ...
            // SPP 장치 포함
            if (self.cachedSPPDevices && self.cachedSPPDevices.length > 0) {
                self.cachedSPPDevices.forEach(function (device) {
                    allDevices.push({
            ...
            // USB 시리얼 장치 포함 (BLE/SPP 존재 여부와 무관하게 항상 조회)
            ...
            bleScan(8,
                ...
                function (devices) {
                    const filtered = devices.filter(function (d) {
                        return d.serviceUuid;
                    });
                    self.cachedBLEDevices = filtered;
                    const mapped = filtered.map(function (d) {
                        return {
                            path: 'ble:' + d.address,
                            displayName: (d.displayName || d.name || d.address || 'Unknown') + ' [BLE]',
                        };
                    });
                ...
                function sendNextFragment() {
                    ...
                    bleWrite(self.connectionId, self.bleServiceUUID, self.bleTxCharUUID,
                        ...
                        function (error) {
                            ...
                            if (_callback) {
                                _callback({ bytesSent: bytesSentSoFar, error: error });
                            }
                            self.outputBuffer.shift();
                            if (self.outputBuffer.length) {
                                _send();
