...
import { installNumericKeypad } from "@/js/numeric_keypad.js";
...
export function startProcess() {
    ...
    if (GUI.isCordova()) {
        UI_PHONES.init();
        installNumericKeypad();
    }

    $('#start-btn').on('click', function (e) {
        e.preventDefault();
        // Collapse the headerbar permanently: from now on the connect tab owns
        // the screen. The header only returns on a fresh app launch.
        ...
        if ($(this).parent().hasClass('active') === false && !GUI.tab_switch_in_progress) { // only initialize when the tab isn't already active
            ...
            GUI.tab_switch_allowed(async function () {
                ...
                GUI.tab_switch_cleanup(function () {
                    ...
                    // Keep the active tab visible in scrollable tab lists
                    // (horizontal bottom bar in portrait, vertical sidebar in
                    // landscape/compact windows). The scroll container has
                    // `scroll-behavior: smooth` so the recentre is animated
                    // (~0.5–1s depending on the distance) instead of jumping
                    // abruptly, while still feeling snappy. Centre on both
                    // axes: `inline:'center'` handles the portrait horizontal
                    // bar, `block:'center'` handles the landscape vertical
                    // sidebar. Each axis is only scrollable in its own
                    // orientation, so the unused axis is a no-op.
                    const activeTab = $(self).parent().get(0);
                    ...
                    if (tabName === 'connect') {
                        $('body').addClass('fc-tab-active-connect');
                    } else if (tabName === 'cli') {
                        $('body').addClass('fc-tab-active-cli');
                    } else {
                        $('body').addClass('fc-tab-active-other');
                    }
    ...
    $('#tabs ul.mode-always li a:first').click();
    ...
    $('#expert-mode input')
        .on('change', function () {
            ...
            // Mirror the state into the options-page toggle when present
            $('#opt-expert-mode').prop('checked', this.checked);
        })
...
export function updateTabList(features) {
    ...
    // GPS 탭: 피처가 활성화 AND 숨김 설정이 아닌 경우에만 표시
    const showGps = features.isEnabled('GPS') && !hidden.includes('gps');
    $('#tabs ul.mode-connected li.tab_gps').toggle(showGps);

    // LED_STRIP 탭: 피처가 활성화 AND 숨김 설정이 아닌 경우에만 표시
    const showLedStrip = features.isEnabled('LED_STRIP') && !hidden.includes('led_strip');
    $('#tabs ul.mode-connected li.tab_led_strip').toggle(showLedStrip);
}
