...
import { showLoadingPopup, hideLoadingPopup, updateProgress } from "@/js/loading_popup.js";
    ...
    if (GUI.connect_lock != true) { // GUI control overrides the user control
        ...
        if (selectedPort.data().isDFU) {
            ...
            if (!clicks) {
                ...
                $('body').addClass('fc-connecting');

                // lock port select & baud while we are connecting / connected
                $('div#port-picker #port, div#port-picker #baud, div#port-picker #delay').prop('disabled', true);
                $('div.connect_controls div.connect_state').text(i18n.getMessage('connecting'));

                // Safety net: if the transport never invokes onOpen (powered-off
                // BLE/SPP devices can hang the native plugin indefinitely),
                // abort the attempt after 15s so the app stays usable.
                GUI.timeout_add('connect_attempt_timeout', function () {
                    if (!CONFIGURATOR.connectionValid && GUI.connecting_to === portName) {
                        GUI.log(i18n.getMessage('connectAttemptTimeout'));
                        GUI.connecting_to = false;
                        $('body').removeClass('fc-connecting');
                        $('body').removeClass('fc-tab-active-connect fc-tab-active-cli fc-tab-active-other');
                        thisElement.data('clicks', false);
...
/**
 * The connect tab hosts the whole connection UI, so it is where the user has to
 * end up whenever the link goes away: on a manual disconnect, on a detected
 * disconnection and after a save & reboot.
 *
 * It is available connected and disconnected and never talks MSP, so it is safe
 * to open even while the FC is rebooting.
 */
export function openConnectTab() {
    // Once the user enters the connect tab, the header log is replaced by the
    // connect tab's own log and must stay hidden on every tab afterwards.
    document.body.classList.add('fc-connect-flow');

    GUI.reboot_in_progress = false;

    // A pending 'dirty' state would pop the save/revert dialog on the way out,
    // but the data was either just saved or the link is already gone.
    if (GUI.current_tab) {
        GUI.current_tab.isDirty = false;
    }

    if (GUI.active_tab === 'connect' && TABS.connect) {
        // Already there: re-render so the tab reflects the new connection state.
        GUI.tab_switch_in_progress = true;
        TABS.connect.cleanup(function () {
            TABS.connect.initialize(function () {
                GUI.tab_switch_in_progress = false;
            });
        });
        return;
...
function finishClose() {
    GUI.timeout_remove('connect_attempt_timeout');
    if (GUI.isCordova()) {
        UI_PHONES.reset();
    }
    ...
    // Show top headerbar/log again (they were hidden while connected)
    $('body').removeClass('fc-connected');
    $('body').removeClass('fc-connecting');
    $('body').removeClass('fc-tab-active-connect fc-tab-active-cli fc-tab-active-other');
...
function finishOpen() {
    ...
    let allowedTabs = GUI.defaultAllowedFCTabsWhenConnected.slice();
    const hidden = config.get('hiddenConnectedTabs') || [];
    if (hidden.length) {
        allowedTabs = allowedTabs.filter(tab => !hidden.includes(tab));
    }
    GUI.allowedTabs = allowedTabs;
...
async function onConnect() {
    ...
    if (FC.CONFIG.flightControllerVersion !== '') {
        ...
        // Show a "Wait for loading..." overlay while the initial post-connect
        // MSP batch is in flight. The overlay is dismissed either by the user
        // tapping anywhere (handled inside loading_popup.js) or once the
        // batchCodes await resolves below. We deliberately do NOT use a
        // timeout here — BLE responses can take tens of seconds, and closing
        // the overlay before the data has actually arrived would defeat its
        // purpose. The overlay shows a "loaded/total" indicator so the user
        // can see how many of the post-connect requests have completed.
        showLoadingPopup();
        try {
...
export function reinitialiseConnection(callback) {
    ...
    // A save & reboot drops the link: park the UI on the connect tab instead of
    // reloading the current tab against a rebooting FC.
    if (CONFIGURATOR.virtualMode) {
        callback?.();
    } else {
        openConnectTab();
