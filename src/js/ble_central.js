...
export function bleConnect(deviceId, onConnect, onDisconnect, onError, deviceHint = null) {
    const ble = ensureNordicBle(onError);
    if (!ble) return;
    ...
    const doConnect = (device) => {
        // Keep the advertising profile found during scan. The native plugin
        // can then skip profile probing and connect directly to the GATT
        // characteristics instead of rediscovering them from the address.
        ble.connect(device, {
            baudRate: 115200,
    ...
    const cached = deviceHint || (ble.devices && ble.devices.find((d) => d.address === address));
    if (cached) {
        doConnect(cached);
    } else {
        ble.getDevices()
        ...
        _extractFrames: function () {
            while (buffer.length >= 6) {
                ...
                if (type === 0x4D) { // V1
                    ...
                    if (sizeByte === 0xFF) {
                        if (buffer.length >= 9) {
                            const size16 = buffer[5] | (buffer[6] << 8);
                            totalLen = 8 + size16;
                        } else break;
                    } else {
                        totalLen = 6 + sizeByte;
