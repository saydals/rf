...
function portRecognized(portName, pathSelect) {
    if (portName) {
            ...
            // Cordova(Android)의 usbevent 플러그인은 "vendorId/productId" 형식의 경로를 반환 (예: "1155/22336")
            const isCordovaUsb = /^\d+\/\d+$/.test(pathSelect);
            const deviceRecognized = portName.includes('STM') || portName.includes('CP210');
            const legacyDeviceRecognized = portName.includes('usb');
            if (isWindows && deviceRecognized || isTty && (deviceRecognized || legacyDeviceRecognized) || isBLE || isSPP || isCordovaUsb) {
                return true;
            }
    }
    return false;
...
PortHandler.check_usb_devices = function (callback) {
    ...
    // Cordova 등 chrome.usb API가 없는 환경에서는 DFU 검사를 건너뜀
    // (가드가 없으면 TypeError로 check() 루프 전체가 중단되어 포트 갱신이 멈춤)
