...
const chromeapiSerial = {
    ...
    // 알려진 USB-Serial 칩 벤더 ID → 표시명 매핑 (portRecognized()가 인식하는 STM/CP210 문자열 포함)
    getDeviceDisplayName: function(vid, pid) {
        switch (vid) {
            case 1155:  // 0x0483 STMicroelectronics (VCP)
                return `STM32 Virtual COM Port (${vid}/${pid})`;
            case 4292:  // 0x10C4 Silicon Labs
                return `CP210x USB-Serial (${vid}/${pid})`;
            case 1027:  // 0x0403 FTDI
                return `FTDI USB-Serial (${vid}/${pid})`;
            case 6790:  // 0x1A86 QinHeng CH340/CH341
                return `CH340 USB-Serial (${vid}/${pid})`;
            default:
                return `${vid}/${pid}`;
        }
    },
    ...
    getDevices: async function(callback) {
        const self = this;
        // usbevent 플러그인이 없는 환경에서는 빈 목록 반환 (예외로 주기적 포트 체크가 중단되는 것을 방지)
        if (typeof cordova === 'undefined' || !cordova.plugins || !cordova.plugins.usbevent) {
            console.warn(`${self.logHeader}usbevent plugin not available`);
            ...
            if (list.devices !== undefined) {
                list.devices.forEach(device => {
                    devices.push({
                        ...
                        displayName: self.getDeviceDisplayName(device.vendorId, device.productId),
                    });
                });
            }
            if (callback) {
