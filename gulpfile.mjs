...
function helper_build_app_cordova() {
  ...
  return gulp.series(
    ...
    cordova_configxml,
    cordova_deps,
    cordova_platform_add,
    cordova_patches,
    cordova_gradle_build,
  );
}
...
// After `cordova platform add` copies the plugin's source into
// platforms/android, pnpm has already wiped our patches to
// node_modules. Re-apply them here so the rebuilt APK contains them.
function cordova_patches() {
  return runAsync(async () => {
    const platformSrc = `${context.appdir}/platforms/android/app/src/main/java/cordova/plugins/screenorientation/CDVOrientation.java`;
    const nodeSrc = `${context.appdir}/node_modules/cordova-plugin-screen-orientation/src/android/CDVOrientation.java`;
    try {
      await fs.access(platformSrc);
      await fs.access(nodeSrc);
    } catch {
      return;
    }
    let content = await fs.readFile(nodeSrc, "utf8");
    if (!content.includes("ALL_SENSOR")) {
      content = content.replace(
        'private static final String LANDSCAPE = "landscape";',
        `private static final String LANDSCAPE = "landscape";
    private static final String ALL_SENSOR = "allSensor";`,
      );
      content = content.replace(
        "} else if (orientation.equals(PORTRAIT_SECONDARY)) {",
        "} else if (orientation.equals(ALL_SENSOR)) {\n            activity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_FULL_SENSOR);\n        } else if (orientation.equals(PORTRAIT_SECONDARY)) {",
      );
      await fs.writeFile(nodeSrc, content);
    }
    await fs.copyFile(nodeSrc, platformSrc);
  });
}

function cordova_platform_add() {
  return runAsync(async () => {
